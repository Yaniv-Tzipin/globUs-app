...
// will listen just to the current user
                    ...
                    child: Column(children: [
                      ...
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            IconButton(
                              onPressed: swipeCardLeftWithButton,
                            ...
                            IconButton(
                              onPressed: swipeCardRightWithButton,
  ...
  bool _onSwipe(
    ...
    (direction == CardSwiperDirection.right)
        ? swipeCardRightWithoutButton(cardsOwnerEmail)
        : swipeCardLeftWithoutButton(cardsOwnerEmail);

    return true;
  }

// will trigger _onSwipe method
  void swipeCardLeftWithButton() async {
    swiperController.swipeLeft();
  }

// will trigger _onSwipe method
  void swipeCardRightWithButton() async {
    swiperController.swipeRight();
  }

  Future<void> swipeCardLeftWithoutButton(String cardsOwnerEmail) async {
    // adding the other user email to the current user swipedLeft list
    _matchService.addSwipedLeft(currentUser.email, cardsOwnerEmail);
  }

  Future<void> swipeCardRightWithoutButton(String cardsOwnerEmail) async {
    //if it's a match the chat service creates a chat room
    bool isAMatch =
        await _matchService.checkIfAMatch(currentUser.email, cardsOwnerEmail);
    if (isAMatch) {
      // adding the other user email to the current user matches list
      await _matchService.addMatch(currentUser.email, cardsOwnerEmail);
      // adding the current user email to the other user matches list
      await _matchService.addMatch(cardsOwnerEmail, currentUser.email);
      // removing the current user email from the other user swipedRight list
      ...
      // adding the other user email to the current user swipedRight list
